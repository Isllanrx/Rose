...
- **CI** (ADR-009): antes de commitar, `python -m ruff check .`, `python -m unittest discover -s test -t .` e `python scripts/quality_gate.py` (instalar `requirements-dev.txt`). Se a catraca mostrar melhora, rodar `--update` no mesmo commit.
- **Nenhum `except` silencioso** (ADR-008): logar (WARNING/ERROR se muda o resultado para o usuário, DEBUG se é fallback), relançar, ou marcar a linha do `except` com `# silent-ok: <motivo>`. Em loop quente, logar uma vez. `test/test_no_silent_exceptions.py` quebra se aparecer handler novo sem isso.
- **Código do cliente é legível**: o JS/templates de cada tela estão em `<League>\Plugins\<rcp-fe-*>\assets.wad`, legíveis com `WadReader` (`entry_hashes()` + `read_hash()`, filtrando por conteúdo). Antes de supor que "o cliente bloqueia X", ler o componente. No Rift Clássico (16.18): a navegação do carrossel depende de `session.allowSkinSelection` do LCU, posse só bloqueia `requestSetSkin`, e `/v1/skin-selector-info` reposiciona o carrossel na skin do LCU.
- **Título da skin só é reenviado pelo SkinMonitor quando muda** (`lastLoggedSkin`, reset só no Lobby). Título que o Python descarta (ex.: antes do lock) não volta sozinho; `ChampThread` (que tinha resync) é código morto (#69).
