...
| 65 | **`lifecycle_manager` le e zera `self.chroma_ui` sem o lock** quando o `acquire` falha (`ui/core/lifecycle_manager.py:202-203`, comentario no proprio codigo: *"risky but necessary"*). Corrida com a thread que usa a referencia: leitura e `None` sem exclusao mutua | concorrencia | **8 ocorrencias numa sessao de 15 min** de teste in-game em 2026-09-19 (`rose_19-09-2026_10-14-04.log`), concentradas nas transicoes de fase (Lobby e InProgress). Nao houve crash, mas o caminho e exercitado toda partida. Correcao minima: `acquire` com timeout maior e, se ainda falhar, **adiar** a limpeza em vez de fazer sem lock |
